...
public class TestFrontend {
        ...
        public void runTests() {
          ...
          System.out.print("Test frontend initially lists at least 1 movie (WARNING: if 'x' does not exit app, test won't exit and run indefinitely!): ");
          if (this.testFrontendInitialOutputWithMovie()) {
                  System.out.println("PASSED");
          } else {
                  System.out.println("FAILED");
          }
       ...
       // TODO: Front End Developer, add at least 2 more tests
          /**
           * This test runs the front end and redirects its output to a string. It then
           * passes in 'r' as a command to go to the genre selection mode. It then exists
           * the app by pressing 'x' to go back to the main mode and another 'x' to exit.
           * The test succeeds if the rating selection screen contains all five ratings
           * from the data. It fails if any of them are missing, the front end has not
           * been instantiated (is null), or there is an exception.
           * @return true if the test passed, false if it failed
           */
          public boolean testFrontEndRForRatings() {
                  ...
                  InputStream standardIn = System.in;
                  try {
                          // set the input stream to our input (with an r to get the ratings)
           ...
           * This test runs the front end and redirects its output to a string. It then
           * passes in 'x' as a command to exit the app. The test succeeds if the front
           * end contains any of the three movies (the movie titles are not in
           * the string captured from the front end) by default. It fails if none of
           * those three titles are present in the string or an exception occurs.
           * @return true if the test passed, false if it failed
           */
          public boolean testFrontendInitialOutputWithMovie() {
                  PrintStream standardOut = System.out;
                  InputStream standardIn = System.in;
                  try {
                          // set the input stream to our input (with an x to test of the program exists)
